...
namespace Stepper
{
    ...
    private:
        inline uint32_t timerTicksFromUs(float timeUs) {
            return static_cast<uint32_t>(timeUs * timerTicksFromUs_);
        };

        inline float timerTicksToUs(uint32_t ticks) {
            return static_cast<float>(ticks) * timerTicksToUs_;
        };
        ...
        static constexpr float timerTicksFromUs_ = static_cast<float>(timerResolutionHz_) / 2e6f;
        static constexpr float timerTicksToUs_ = 2e6f / static_cast<float>(timerResolutionHz_);
